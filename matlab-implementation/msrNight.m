function R = msrNight(I, sigmas, weights)
...
for i = 1:size(I, 3)
    spectralBandi = I(:,:,i);
    spectralBandi(spectralBandi == 0) = 0.01;   % Make log friendly :)
    
    Ri = zeros(size(spectralBandi));
    
    for j = 1:size(sigmas, 2)
        Li = imgaussfilt(spectralBandi, sigmas(j));
        Li(Li == 0) = 0.01;
        Ri = Ri + weights(j) * (sigCustom(spectralBandi,k) - sigCustom(Li,k));
    end
